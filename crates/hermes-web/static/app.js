...
async function init() {
    ...
    var tl = document.getElementById('tool-list');
    tl.innerHTML = ['shell','read_file','write_file','grep','git','web_fetch']
        .map(function(t) { return '<span>' + t + '</span>'; }).join('');
    var cfg = '<b>Model:</b> ' + d.model + '<br>' +
        '<b>Version:</b> ' + d.version + '<br>' +
        '<b>API Key:</b> ' + (d.api_key ? 'set' : 'missing');
    document.getElementById('config-info').innerHTML = cfg;
}
...
function addMsg(role, content, cls) {
    ...
    div.innerHTML = '<div class="role">' + (role === 'user' ? 'You' : 'Hermes') +
        '</div><div class="content">' + escHtml(content) + '</div>';
...
function showTab(name) {
    ...
    var el = document.querySelector('nav a[onclick*="' + name + '"]');
